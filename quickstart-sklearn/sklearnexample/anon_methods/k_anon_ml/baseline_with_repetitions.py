...
def main(args):
    ...
    if results_path == '':
        results_path = f'.results/{anon_method}'
    # Dataset path
    ...
    res_folder = os.path.join('results', dataset, anon_method)


    cluster_gen_method = ''
    if anon_method == AnonMethod.GFKMC:
        cluster_gen_method = '_most_common_register'
        # cluster_gen_method = '_most_common_values'
        # cluster_gen_method = ''



    # ML results path
    if anon_method == AnonMethod.ORIGINAL:
        output_path = os.path.join(res_folder, f'{dataset}_{os.path.basename(anon_method)}_{os.path.basename(classifier)}.csv')
    else:
        output_path = os.path.join(res_folder, f'{dataset}_{os.path.basename(anon_method)}{cluster_gen_method}_{os.path.basename(classifier)}_k_{args.stop_k}.csv')
    ...
    elif dataset == Dataset.ADULT_CAT:
        QI_INDEX = [1, 2, 3, 4, 5, 6, 7]
        target_var = 'salary-class'
        IS_CAT2 = [True, True, True, True, True, True, True]
        # max_numeric = {"age": 50.5}
    ...
    # csv_file = f'./{res_folder}/{dataset}_anon_metrics_ncp.csv'
    ...
    for k in k_range:
        ...
        if anon_method == AnonMethod.MONDRIAN:
            anon_data = get_result_one(ATT_TREES, raw_data, k, path, QI_INDEX, SA_INDEX, f'{results_path}/ncp.txt')
        elif anon_method == AnonMethod.TDG:
            anon_data = tdg_get_result_one(ATT_TREES, raw_data, k, path, QI_INDEX, SA_INDEX, f'{results_path}/ncp.txt')
        elif anon_method == AnonMethod.CB:
            anon_data = cb_get_result_one(ATT_TREES, raw_data, k, path, QI_INDEX, SA_INDEX, args.cb_alg, f'{results_path}/ncp.txt')
        ...
        if anon_method != AnonMethod.GFKMC and anon_method != AnonMethod.ORIGINAL:
            # if not os.path.exists(anon_csv):
            # Write anonymized data in csv file
            nodes_count = write_anon(results_path, anon_data, header, k, s, dataset, ',')
            anon_data = pd.read_csv(os.path.join(
                results_path, f"{dataset}.csv"), delimiter=',')
            anon_data = anon_data.sort_values(by=['ID'])
            anon_data.drop("ID", axis=1).to_csv(os.path.join(
                results_path, f"{dataset}.csv"), index=False)
        ...
        #         anon_data = pd.read_csv(f'../results/{dataset}/gfkmc{cluster_gen_method}/{dataset}_anon_k{k}.csv')
        #         anon_data['ID'] = anon_data.index
        #     elif anon_method == AnonMethod.ORIGINAL:
        #         anon_data = pd.read_csv(f'../datasets/adult_orig.csv')
        #         anon_data['ID'] = anon_data.index
        #     else:
        #         # reading in the anonymized data
        #         # anon_data = pd.read_csv(os.path.join(
        #         #     s_folder, dataset + "_anon_k" + str(k) + ".csv"), delimiter=';')
        #         anon_data = pd.read_csv(os.path.join(
        #             s_folder, f"{dataset}_anon_k{k}.csv"), delimiter=',')
        #         anon_data['ID'] = anon_data.index
        #         print(
        ...
        #         # anon_data = anon_data.sort_values(by=['ID'])
        #         # anon_data.drop("ID", axis=1).to_csv(os.path.join(
        #         #     s_folder, f"{dataset}_anon_k{k}.csv"), index=False)
        ...
        #             if col == '*' and dataset != Dataset.ADULT_CAT:
        ...
        #                 if newval is None  and dataset != Dataset.ADULT_CAT:
        #                     print('Err: ' + max_numeric.get(index_col) + "   index: " + index_col)
        #                 X.at[index_row, index_col] = newval
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset-path', default="", help='dataset path')
    parser.add_argument('--results-path', default="", help='results path')
    ...
    parser_mon = subparsers.add_parser(AnonMethod.GFKMC.value, help='gfkmc anonyization algorithm')
    parser_mon = subparsers.add_parser(AnonMethod.ORIGINAL.value, help='original adult dataset')
